...
public class DocumentServiceImpl implements DocumentService {
	...
	public final static Log logger = LogFactory.getLog(DocumentServiceImpl.class);
	...
	private boolean canHandleCSV(Exception e) {
		if (e != null) {
			if (e.getMessage().contains("Invalid header signature;")) {
				...
				// an exception will be thrown in the HeaderBlockReader class (calling from the constructor of POIFSFileSystem):
	...
	private DocumentOutput handleCSV(DocumentParameter parameter) {
		...
		try {
			...
			if (parameter.getDelimiter() != null) {
				...
				reader = new CSVReader(new InputStreamReader(new ByteArrayInputStream(parameter.getData())), separator);
			} else {
...
//	}
	...
	private void removeEmptyColumns(List<String> columnNameList, List<Map<String,String>> data, boolean[] columnsHaveData) {
...
//				break;
	...
	public DocumentExportOutput export(DocumentExportInput input) {
		...
		de.setFileName("export-file-"+sdf.format(new Date())+"."+getFileNameExtension(input.getExportCode()));
