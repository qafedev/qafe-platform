...
public class TableCellUIAssembler implements UIAssembler {

	public TableCellUIAssembler(){}
	...
	public ComponentGVO convert(Component object, Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof TableCell) {
				...
				TableCellGVO voTemp  = new TableCellGVO();
				UIAssemblerHelper.copyFields(tableCell, currentWindow,voTemp,applicationMapping, context, ss);
				voTemp.setCellSpan(tableCell.getCellSpan());
				voTemp.setRowSpan(tableCell.getRowSpan());
				voTemp.setComponent(ComponentUIAssembler.convert(tableCell.getComponent(), currentWindow,applicationMapping,context, ss));
