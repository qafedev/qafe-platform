...
public class AreaUIAssembler implements UIAssembler {

	
	public AreaUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object,Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof Area) {
				...
				AreaGVO voTemp  = new AreaGVO();
				UIAssemblerHelper.copyFields(area, currentWindow, voTemp,applicationMapping, context, ss);			
				voTemp.setAlt(area.getAlt());
				voTemp.setCoords(area.getCoords());
				voTemp.setShape(area.getShape());
				vo =voTemp;
