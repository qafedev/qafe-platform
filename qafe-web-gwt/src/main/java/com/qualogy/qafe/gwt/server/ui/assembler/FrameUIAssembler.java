...
public class FrameUIAssembler implements UIAssembler {

	public FrameUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object, Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof FrameComponent) {
				FrameComponent frame = (FrameComponent)object;
				FrameGVO voTemp  = new FrameGVO();
				UIAssemblerHelper.copyFields(frame, currentWindow,voTemp,applicationMapping, context, ss);				
				voTemp.setSrc(frame.getSrc());								
				vo =voTemp;
			}
		}
		return vo;
