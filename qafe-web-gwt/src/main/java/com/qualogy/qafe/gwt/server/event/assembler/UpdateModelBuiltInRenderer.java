...
public class UpdateModelBuiltInRenderer extends AbstractInternalBuiltInRenderer {
	...
	public BuiltInFunctionGVO convert(EventItem eventItem, EventDataGVO eventData, ApplicationContext context, SessionContainer sc) {
		...
		if (eventItem instanceof UpdateModel) {
			UpdateModel updateModel = (UpdateModel)eventItem;
			
			UpdateModelGVO updateModelGVO = new UpdateModelGVO();			
			bif = updateModelGVO;
			fillIn(updateModel, updateModelGVO, eventData);
			
			updateModelGVO.setRef(parseComponent(updateModel.getRef(), eventData));
			updateModelGVO.setUpdateAction(updateModel.getUpdateAction());
		}
		return bif;
	}
}
