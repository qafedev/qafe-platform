 ...
 *
 */
public class ApplicationContext implements Serializable, PostProcessing, AuthenticationControlled, AuthorizationControlled{
	
	private static final long serialVersionUID = 6623946831871605896L;
	/**
	 * Private datamember to keep track of load errors. This is especially interesting
	 * when a big application-config, with more than one application fails. In the previous
	 * implementation when an error occured (it doesn't matter which of the applications) all
	 * the applications would not load also. This is quite tedious and not very friendly.
	 * So if an application failes to load, the rootCause message will be stored in this variable.
	 */
	...
	///////////// mappings ///////////////////
	/**
	 * the final product of a merge between interpreted mappingFileLocations
	 * and assignedMappings, the public getApplicationMapping on this class
	 * will return this merged mapping. 
	 */
	...
	/**
	 * String which can contain the filepath to the application context file
	 * this application context origins from 
	 */
	...
	/**
	 * boolean property can be used to set auto-scan-root property true saying
	 * automatic scan root directory if no application-mapping is set. (false
	 * means don't scan (default))
	 */
	...
	public void addFileLocation (FileLocation fileLocation){
		if (mappingFileLocations == null) mappingFileLocations = new ArrayList<FileLocation>();
	...
	/**
	 * method called after binding of the context file. method takes care of merging and binding
	...
	public void performPostProcessing() {
		try {
			...
			System.err.println("ApplicationContext [id=" + getId()+",name="+ getName()+"] failed to load because of: ");
	...
	/**
	 * Method to setup default settings for the ApplicationContext. This
	 * method will not overwrite existing settings, but will fill the ones
	 * that are null.
	 */
	public void setDefaults(){
		if(this.applicationConfiguration==null)
			...
			this.applicationConfiguration.fillInTheBlanks(ConfigurationDefaults.getInstance().getAppConfiguration());
			this.applicationConfiguration.fillInTheBlanks(ConfigurationDefaults.getInstance().getGlobalConfiguration());
		}
	}
