...
public class HiddenUIAssembler implements UIAssembler {

	public HiddenUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object, Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof Hidden) {
				Hidden hidden = (Hidden)object;
				HiddenGVO voTemp  = new HiddenGVO();
				UIAssemblerHelper.copyFields(hidden, currentWindow,voTemp,applicationMapping, context, ss);				
				voTemp.setValue(hidden.getValue());								
				vo =voTemp;
			}
		}
		return vo;
