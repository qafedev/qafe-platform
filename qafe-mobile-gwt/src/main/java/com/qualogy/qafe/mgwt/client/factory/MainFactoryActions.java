...
public class MainFactoryActions {
	...
	public static void getUIByUUID(String uuid, final String windowId) {
		UIGVO uiPre = ClientApplicationContext.getInstance().getPrerenderedUIVO(uuid);
		
		if (uiPre==null){
			ClientApplicationContext.getInstance().setBusy(true);
			RPCServiceAsync service = getRPCService();
	...
	public static void processUIFromApplicationContext(final ClientFactory clientFactory) {
		ClientApplicationContext.getInstance().setBusy(true);
		RPCServiceAsync service = getRPCService();
		AsyncCallback<?> callback = createMultipleGVOCallBack(clientFactory);
		service.getUISFromApplicationContext(ClientApplicationContext.getInstance().getParameters(),callback);
	}

	public static void processReloadUIFromApplicationContext() {
		ClientApplicationContext.getInstance().setBusy(true);
		RPCServiceAsync service = getRPCService();
		AsyncCallback<?> callback = createMultipleGVOCallBack(null);
		service.reload(ClientApplicationContext.getInstance().getParameters(),callback);
	}
	...
	private static AsyncCallback<?> createCallBack() {
		if(callback == null) {
			callback =  new AsyncCallback<Object>() {
				...
				public void onFailure(Throwable caught) {
					ClientApplicationContext.getInstance().log("Processing screen creation failed", caught.getMessage(), true, true, caught);
					ClientApplicationContext.getInstance().setBusy(false);
					updateTime(null);
				}
			};
	...
	private static AsyncCallback<?> createMultipleGVOCallBack(final ClientFactory clientFactory) {
				...
				public void onSuccess(Object result) {
					if (result != null) {
						try {
							...
							ClientApplicationContext.getInstance().setGlobalConfigurations(ui.getExternalProperties());
							ClientApplicationContext.getInstance().clearPrerendering();
							if (clientFactory!=null){
								clientFactory.setUIModel(ui);
								
							}
//							if (ui.getVos() != null) {
//								if (ui.getVos().length>0){
//									for (int i = 0; i < ui.getVos().length; i++) {
//										WindowFactory.createWindow(ui.getVos()[i]);
//										updateTime(ui.getVos()[i]);
//									}
//									if (ui.getCss() != null && ui.getCss().length() > 0) {
//										if (isIE()) {
//											addStyleSheet(ui.getCss());
//										} else {
//											Element ele = ClientApplicationContext.getInstance().getStyleElement();
//											if (ele != null) {
...
//									}
//								} else {
//									ClientApplicationContext.getInstance().log("Warning: Getting intial state results in no applications!", "There seems to be no application deployed. Please check if the following: \n- web.xml (is there an application-config.xml), \n- does the application-config.xml exist ? \n- Are there applications defined in the application-config.xml and are the referenced files available ?" +"", true, false,null);
//								}
//							}
	...
	public static void updateTime(UIGVO ui) {
		...
		ClientApplicationContext.getInstance().setLogText("Execution time:" + time + " ms");
	}
	...
	public static void removeUploadedFile(String appUUID, String windowId, String uploadUUID) {
		RPCServiceAsync service = getRPCService();
		AsyncCallback<?> callback = createRemoveCallBack();
		service.removeFileFromLocalStore(appUUID, windowId, uploadUUID, callback);
	}
	...
	private static AsyncCallback<?> createRemoveCallBack() {
		if(removeCallBack == null){
			removeCallBack = new AsyncCallback<Object>() {
				public void onFailure(Throwable caught) {
					ClientApplicationContext.getInstance().log("Removing uploaded file failed", caught.getMessage(), true, true, caught);
				}
				public void onSuccess(Object result) {
				}
			};
	...
	private static AsyncCallback<?> createNotificationCallBack() {
		if(notificationCallBack == null){
			notificationCallBack = new AsyncCallback<Object>() {
				public void onSuccess(Object result) {
					ClientApplicationContext.getInstance().log(TITLE_MESSAGE_IS_SENT, MESSAGE_ERROR_REPORT_SEND_TO_SUPPORT, false, true, null);
				}
				public void onFailure(Throwable caught) {
					ClientApplicationContext.getInstance().log(TITLE_NO_REPORT_SERVICE_AVAILABLE, caught.getMessage() + ". " + MESSAGE_SEND_EMAIL_TO_SUPPORT, true, true, caught);
				}
			};
		}
