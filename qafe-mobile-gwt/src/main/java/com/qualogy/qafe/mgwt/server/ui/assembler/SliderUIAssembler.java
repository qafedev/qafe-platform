...
public class SliderUIAssembler implements UIAssembler {

	public SliderUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object, Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof Slider) {
				...
				SliderGVO voTemp  = new SliderGVO();
				UIAssemblerHelper.copyFields(slider, currentWindow,voTemp,applicationMapping, context, ss);				
				voTemp.setMaxTicks(slider.getMaxTicks());
				voTemp.setMinTicks(slider.getMinTicks());
				voTemp.setOrientation(slider.getOrientation());
				voTemp.setDisplayname(slider.getDisplayname());
				voTemp.setTickSize(slider.getTickSize());
				voTemp.setTickLabels(slider.getTickLabels());
				voTemp.setValue(slider.getValue());
				vo =voTemp;
			}
		}
		return vo;
