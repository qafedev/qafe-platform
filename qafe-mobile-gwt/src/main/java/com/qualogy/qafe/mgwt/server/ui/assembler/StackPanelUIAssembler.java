...
public class StackPanelUIAssembler implements UIAssembler {

	public StackPanelUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object, Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof StackPanel) {
				StackPanel stackPanel = (StackPanel)object;
				StackPanelGVO voTemp  = new StackPanelGVO();
				UIAssemblerHelper.copyFields(stackPanel, currentWindow,voTemp,applicationMapping, context, ss);		
			
				if (stackPanel.getStacks()!=null){
					StackGVO[] stacks = new StackGVO[stackPanel.getStacks().size()];
					int index=0;
					for (Stack stack : stackPanel.getStacks()) {
						stacks[index]= (StackGVO)ComponentUIAssembler.convert(stack, currentWindow,applicationMapping,context, ss)  ;
						index++;
