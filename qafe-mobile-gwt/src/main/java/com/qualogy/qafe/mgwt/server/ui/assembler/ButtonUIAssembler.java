...
public class ButtonUIAssembler implements UIAssembler {

	
	public ButtonUIAssembler() {
	}
	...
	public ComponentGVO convert(Component object,Window currentWindow,ApplicationMapping applicationMapping, ApplicationContext context, SessionContainer ss) {
		...
		if (object != null) {
			if (object instanceof Button) {
				...
				ButtonGVO voTemp  = new ButtonGVO();
				UIAssemblerHelper.copyFields(button, currentWindow,voTemp,applicationMapping, context, ss);			
				voTemp.setLabel(button.getDisplayname());
				voTemp.setImageLocation(button.getImageLocation());
				vo =voTemp;
			}
		}
		return vo;
